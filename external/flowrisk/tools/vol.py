...
class RecursiveEWMAVol(Vol):
    ...
    def __init__(self, decay, value_type):
        ...
        """
        assert (
            isinstance(decay, float) and 0.0 <= decay <= 1.0
    ...
    def check_initial_args(initial_price_or_prices, initial_vol):
        ...
        assert (
            isinstance(initial_vol, float) or initial_vol is None
    ...
    def check_prices(prices):
        assert len(prices) > 1, "prices should have more than one price"
        assert len(prices.shape) == 1, "prices should be one-dimensional vector"
        assert (
            prices.dtype == "float64" or prices.dtype == "float32"
    ...
    def initialize_with_prices(self, prices):
        ...
        if self.value_type == "pnl":
            squared_log_returns = np.square(prices[1:] - prices[:-1])
        elif self.value_type == "simple":
            squared_log_returns = np.square(prices[1:] / prices[:-1] - 1.0)
        elif self.value_type == "log":
            squared_log_returns = np.square(np.log(prices[1:] / prices[:-1]))
        else:
            raise ValueError('return_type should be "simple" or "log"')
        accumulator = (
            lambda prev_ret, curr_ret: self.decay * prev_ret
            + (1.0 - self.decay) * curr_ret
        )
    ...
    def check_price(self, price):
        ...
        assert self.latest_price is not None, "Has not initialized the initial price"
    ...
    def update(self, price):
        ...
        assert self.latest_price is not None, (
            "RecursiveEWMAVol has not been initialized. " "The latest price is None."
        )

        if self.value_type == "pnl":
            if self.latest_var is not None:
                self.latest_var = (1.0 - self.decay) * (
                    price - self.latest_price
                ) ** 2.0 + self.decay * self.latest_var
            else:
                self.latest_var = (price - self.latest_price) ** 2.0
        elif self.value_type == "simple":
            if self.latest_var is not None:
                self.latest_var = (1.0 - self.decay) * (
                    price / self.latest_price - 1.0
                ) ** 2.0 + self.decay * self.latest_var
            else:
            ...
            if self.latest_var is not None:
                self.latest_var = (
                    (1.0 - self.decay)
                    * np.math.log(
                        price / (self.latest_price + self.EPSILON) + self.EPSILON
                    )
                    ** 2.0
