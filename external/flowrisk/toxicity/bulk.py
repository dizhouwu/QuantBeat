...
class RecursiveBulkClassMABuckets(Buckets):
    def __init__(self, bucket_max_volume, n_bucket):
        ...
        """
        assert isinstance(bucket_max_volume, int) or isinstance(
            bucket_max_volume, float
        ), "bucket_max_volume should be an int"
        self.bucket_max_volume = bucket_max_volume
        assert isinstance(n_bucket, int), "n_bucket should be an int"
    ...
    @staticmethod
    def check_initial_args(
        initial_price_or_prices, pnl_volatility_estimate, initial_volumes
    ):
        assert isinstance(initial_price_or_prices, float) or isinstance(
            initial_price_or_prices, np.ndarray
        ), "initial_price_or_prices should be a numpy.ndarray or a float"
        if isinstance(initial_price_or_prices, float):
            assert (
                initial_price_or_prices >= 0.0
            ), "initial_price_or_prices should be non-negative if it is a float"
        assert (
            isinstance(pnl_volatility_estimate, float)
            or pnl_volatility_estimate is None
        ), "pnl_volatility_estimate should be a float or None"
        if isinstance(pnl_volatility_estimate, float):
            assert (
                pnl_volatility_estimate >= 0.0
            ), "pnl_volatility_estimate should be non-negative if it is a float"
        assert (
            isinstance(initial_volumes, np.ndarray) or initial_volumes is None
        ), "initial_vol should be a numpy.ndarray or None"

    def initialize(
        self,
        initial_price_or_prices,
        pnl_volatility_estimate=None,
    ...
    def check_prices_and_volumes(initial_prices, initial_volumes):
        assert (
            len(initial_prices) >= 1
        ), "initial_prices should have more than one price"
        assert (
            len(initial_prices.shape) == 1
        ), "initial_prices should be one-dimensional vector"
        assert (
            initial_prices.dtype == "float64" or initial_prices.dtype == "float32"
        ), "the datatype of initial_prices should be float32 or float64"
        assert (
            len(initial_volumes) >= 1
        ), "initial_volumes should have more than one price"
        assert (
            len(initial_volumes.shape) == 1
        ), "initial_volumes should be one-dimensional vector"
        assert (
            initial_volumes.dtype == "float64" or initial_volumes.dtype == "float32"
        ), "the datatype of initial_volumes should be float32 or float64"

    def initialize_with_prices_and_volumes(
        self, initial_prices, pnl_volatility_estimate, initial_volumes
    ):
        """
    ...
    def get_order_imbalance(self, idx=None):
        ...
        """
        if idx is None:
            return np.abs(self.bucket_buy_volume_array - self.bucket_sell_volume_array)
        else:
            return np.abs(
                self.bucket_buy_volume_array[idx] - self.bucket_sell_volume_array[idx]
            )
    ...
    def move_to_next_bucket(self):
        ...
        self.current_bucket_volume_from_last_round = self.bucket_volume_array[
            self.current_bucket_idx
        ]
        self.current_bucket_buy_volume_from_last_round = self.bucket_buy_volume_array[
            self.current_bucket_idx
        ]
        self.current_bucket_sell_volume_from_last_round = self.bucket_sell_volume_array[
            self.current_bucket_idx
        ]
    ...
    def update(self, price, pnl_volatility_estimate, volume):
        ...
        buy_volume_pct = norm.cdf((price - self.latest_price) / pnl_volatility_estimate)
        ...
        while time_bar_remaining_volume > 0:
            ...
            if time_bar_remaining_volume < bucket_remaining_volume:
                self.bucket_volume_array[
                    self.current_bucket_idx
                ] += time_bar_remaining_volume
                buy_volume = np.math.floor(buy_volume_pct * time_bar_remaining_volume)
                self.bucket_buy_volume_array[self.current_bucket_idx] += buy_volume
                self.bucket_sell_volume_array[self.current_bucket_idx] += (
                    time_bar_remaining_volume - buy_volume
                )
                ...
                buy_volume = np.math.floor(buy_volume_pct * bucket_remaining_volume)
                self.bucket_buy_volume_array[self.current_bucket_idx] += buy_volume
                self.bucket_sell_volume_array[self.current_bucket_idx] += (
                    bucket_remaining_volume - buy_volume
                )
...
class RecursiveBulkClassEWMABuckets(Buckets):
    def __init__(self, bucket_max_volume, past_bucket_decay):
        ...
        self.bucket_max_volume = bucket_max_volume
        assert (
            isinstance(past_bucket_decay, float) and 0.0 <= past_bucket_decay <= 1.0
    ...
    def move_to_next_bucket(self):
        ...
        if self.running_flag:
            self.accumulated_order_imbalance = (1.0 - self.past_bucket_decay) * np.abs(
                self.current_bucket_buy_volume - self.current_bucket_sell_volume
            ) + self.past_bucket_decay * self.accumulated_order_imbalance
        else:
    ...
    def update(self, price, pnl_volatility_estimate, volume):
        ...
        while time_bar_remaining_volume > 0:
            bucket_remaining_volume = (
                self.bucket_max_volume - self.current_bucket_volume
            )
            if time_bar_remaining_volume < bucket_remaining_volume:
