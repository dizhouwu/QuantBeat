...
import nonlinshrink.nonlinshrink_numba as nls_numba
...
def test_analytic_shrinkage():
    ...
    expected = np.array([[1.04344299, 0.0335051], [0.0335051, 1.11112703]])
    sigma_tilde = nls_numba.shrink_cov(x, 0)

    assert np.allclose(sigma_tilde, expected)
...
def test_demean():
    ...
    sigma_tilde = nls_numba.shrink_cov(data)
    S = (
        np.sum(sigma_tilde[np.eye(p) == 0])
        / n_choose_k(p, 2)
        / np.sum(np.diag(sigma_tilde))
        * p
    )
...
def test_ols():
    ...
    sigma = np.eye(p, p)
    data = (
        np.random.multivariate_normal(np.zeros(p), sigma, n)
    ...
    res = data - datahat
    sigma_tilde = nls_numba.shrink_cov(
        res, k=2
    )  # corresponding to 2 degrees of freedom
    S = (
        np.sum(sigma_tilde[np.eye(p) == 0])
        / n_choose_k(p, 2)
        / np.sum(np.diag(sigma_tilde))
...
def test_singular():
    ...
    with pytest.raises(ValueError):
        sigma_tilde = nls_numba.shrink_cov(data, 0)
...
def test_numba_match_no_numba():
    p = 13
    n = 12
    sigma = np.eye(p, p)
    data = np.random.multivariate_normal(np.zeros(p), sigma, n)
    without_numba = nls.shrink_cov(data, 0)
    with_numba = nls_numba.shrink_cov(data, 0)
    assert np.allclose(with_numba, without_numba)
